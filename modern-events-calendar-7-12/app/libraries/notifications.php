...
class MEC_notifications extends MEC_base
{
    ...
    */
    ...
    public function booking_moved($book_id)
    {
        ...
        if(!isset($this->notif_settings['booking_moved']['status']) || !$this->notif_settings['booking_moved']['status']) return false;

        // Event ID
        $event_id = get_post_meta($book_id, 'mec_event_id', true);

        $subject = isset($this->notif_settings['booking_moved']['subject']) ? esc_html__($this->notif_settings['booking_moved']['subject'], 'mec') : esc_html__('Your booking has been rescheduled.', 'mec');
        $subject = $this->content($this->get_subject($subject, 'booking_moved', $event_id, $book_id), $book_id);

        $headers = array('Content-Type: text/html; charset=UTF-8');

        $recipients_str = $this->notif_settings['booking_moved']['recipients'] ?? '';
        $recipients = trim($recipients_str) ? explode(',', $recipients_str) : [];

        $users = $this->notif_settings['booking_moved']['receiver_users'] ?? [];
        $users_down = $this->main->get_emails_by_users($users);
        $recipients = array_merge($users_down, $recipients);

        $roles = $this->notif_settings['booking_moved']['receiver_roles'] ?? [];
        $user_roles = $this->main->get_emails_by_roles($roles);
        $recipients = array_merge($user_roles, $recipients);
        ...
        foreach($attendees as $attendee)
        {
            ...
            $message = $this->notif_settings['booking_moved']['content'] ?? '';
